...
(function () {
  ...
  var MY_LIBRARY_KEY = "recurrence-lab:my-library:v1";
  ...
  function ready() {
    ...
    var presetEl = $("cfgPreset");
    if (presetEl) {
      presetEl.addEventListener("change", function () {
        var v = presetEl.value;
        if (v !== "custom" && presetFromValue(v)) {
          var p = presetFromValue(v);
          el.cfg.min.value = p.poolMin;
          el.cfg.max.value = p.poolMax;
          el.cfg.picks.value = p.picks;
          readConfig();
          onInput();
        } else if (v === "custom") {
          // Manual fields stay as-is; user edits pool/min/max/picks freely.
          readConfig();
          onInput();
        }
      });
    ...
    // Library: fetch manifest + render cards, then wire one-click load.
    initLibrary();
    // My library: render any user-saved datasets from localStorage.
    renderMyLibrary();
  ...
  function initLibrary() {
    ...
    if (!grid) return; // Library not present in this build
    var refresh = $("btnRefreshLibrary");
    if (refresh) {
  ...
  function loadLibraryManifest(force) {
    var grid = $("libraryGrid");
    if (!grid) return;
    library.status = "loading";
    grid.innerHTML = '<div class="library-empty">Loading library&hellip;</div>';
    // Resolve manifest path relative to the document base.
    var url = resolveDatasetUrl("datasets/manifest.json");
    fetch(url, { cache: force ? "no-cache" : "default" })
      ...
      .then(function (manifest) {
        ...
        renderLibraryCards(manifest.datasets || []);
      })
      .catch(function (err) {
        library.status = "error";
        grid.innerHTML =
          '<div class="library-empty">Could not load the dataset library. ' +
          esc(String(err.message || err)) +
          '<br><span class="hint">If you opened app.html directly from disk, ' +
          "the browser blocks fetch() of local files. Run it from a static server " +
          "(Netlify, GitHub Pages, or <code>npx serve</code>) so the library can load.</span></div>";
      });
  }

  function renderLibraryCards(datasets) {
    var grid = $("libraryGrid");
    if (!grid) return;
    if (!datasets.length) {
      ...
      return (
        '<div class="library-card" role="listitem" data-dsid="' + esc(d.id) + '">' +
          ...
          '<div class="lc-source">Source: ' +
            (d.sourceUrl
              ? '<a href="' + esc(d.sourceUrl) + '" target="_blank" rel="noopener noreferrer">' + esc(d.source) + "</a>"
              : esc(d.source || "")) +
            " &middot; accessed " + esc(d.accessed || "") +
          "</div>" +
          '<button class="primary-button lc-load" type="button" data-load="' + esc(d.id) + '">Load dataset</button>' +
          '<div class="lc-status" data-status="' + esc(d.id) + '" hidden></div>' +
        "</div>"
      );
    }).join("");
    // Wire load buttons
  ...
  function loadDatasetById(id, btn) {
    var ds = (library.manifest && library.manifest.datasets || []).filter(function (x) { return x.id === id; })[0];
    if (!ds) return;
    var statusEl = document.querySelector('[data-status="' + cssEscape(id) + '"]');
    if (btn) { btn.disabled = true; btn.textContent = "Loading…"; }
    if (statusEl) { statusEl.hidden = false; statusEl.textContent = "Loading…"; }
    var url = resolveDatasetUrl(ds.file);
    fetch(url)
      .then(function (r) {
        if (!r.ok) throw new Error("HTTP " + r.status);
        return r.text();
      ...
      .then(function (text) {
        // Populate the textarea and apply the dataset's config.
        el.input.value = text;
        applyDatasetConfig(ds);
        onInput();
        // Mark active card
        Array.prototype.forEach.call(document.querySelectorAll(".library-card"), function (c) {
          c.classList.remove("is-active");
        });
        var card = document.querySelector('.library-card[data-dsid="' + cssEscape(id) + '"]');
        if (card) card.classList.add("is-active");
        if (btn) { btn.disabled = false; btn.textContent = "Loaded ✓"; }
        if (statusEl) {
          statusEl.textContent = "Loaded " + ds.draws + " draws (" + ds.game + "). Click Run analysis.";
        }
        // Scroll to the analysis button so the user sees the next step.
        var runBtn = $("btnAnalyze");
        if (runBtn && runBtn.scrollIntoView) runBtn.scrollIntoView({ behavior: "smooth", block: "center" });
      })
      .catch(function (err) {
        if (btn) { btn.disabled = false; btn.textContent = "Load dataset"; }
        if (statusEl) {
  ...
  // Apply a dataset's config to the intake controls. For named presets use the
  // preset dropdown; for custom configs (Powerball, Mega Millions, UK Lotto) set
  // the manual fields directly and switch the dropdown to "custom".
  function applyDatasetConfig(ds) {
    if (ds.preset && ds.preset !== "custom" && presetFromValue(ds.preset)) {
      var p = presetFromValue(ds.preset);
      el.cfg.min.value = p.poolMin;
      el.cfg.max.value = p.poolMax;
      el.cfg.picks.value = p.picks;
      var presetEl = $("cfgPreset");
      if (presetEl) presetEl.value = ds.preset;
    } else {
      // Custom config from the manifest.
      el.cfg.min.value = ds.poolMin;
      el.cfg.max.value = ds.poolMax;
      el.cfg.picks.value = ds.picks;
      var presetEl2 = $("cfgPreset");
      if (presetEl2) presetEl2.value = "custom";
    }
    readConfig();
  }

  // Resolve a dataset path relative to the document, whether the app is loaded
  // from index.html (root) or app.html (root) or a nested share iframe.
  function resolveDatasetUrl(path) {
    // Use the document's base URL; fetch is relative to the document.
    // In the share iframe the document is app.html at root, so relative works.
    return path;
  }
  ...
  function getMyLibrary() {
    try {
      var raw = localStorage.getItem(MY_LIBRARY_KEY);
  ...
  function setMyLibrary(arr) {
    try {
      localStorage.setItem(MY_LIBRARY_KEY, JSON.stringify(arr));
    } catch (e) {
      // Quota or serialization error — surface to the user.
      alert("Could not save to my library: " + (e.message || e));
    }
  }

  function genMyId() {
    return "my-" + Date.now().toString(36) + "-" + Math.floor(Math.random() * 1e4).toString(36);
  }

  function openSaveDialog() {
    if (!state.parsed || !state.parsed.draws.length) return;
    var existing = document.querySelector(".save-dialog");
    if (existing) existing.remove();

    var cfg = state.config;
    var n = state.parsed.draws.length;
    var dated = state.parsed.datedDrawsCount;
    var firstDate = null, lastDate = null;
    state.parsed.draws.forEach(function (d) {
      if (d.date) { if (!firstDate || d.date < firstDate) firstDate = d.date; if (!lastDate || d.date > lastDate) lastDate = d.date; }
    });
    var range = (firstDate && lastDate) ? (firstDate + " to " + lastDate) : "no dates";

    var dlg = document.createElement("div");
    dlg.className = "save-dialog";
    dlg.innerHTML =
      '<form id="saveForm">' +
        "<h4>Save to my library</h4>" +
        "<p>Stores the current draw history in this browser's localStorage. Not uploaded, not shared.</p>" +
        '<div class="save-summary">' +
          n + " draws &middot; " + cfg.picks + "/" + cfg.poolSize +
          (cfg.bonusPool ? " + bonus" : "") + " &middot; " + range +
          (dated ? " &middot; " + dated + " dated" : "") +
        "</div>" +
        '<input type="text" id="saveName" placeholder="Dataset name (e.g. My 6/49 tracking)" maxlength="60" required autofocus>' +
        '<div class="save-actions">' +
          '<button type="button" class="ghost-button" id="saveCancel">Cancel</button>' +
    ...
    $("saveCancel").addEventListener("click", function () { dlg.remove(); });
    $("saveForm").addEventListener("submit", function (ev) {
      ev.preventDefault();
      var name = (nameInput && nameInput.value || "").trim();
      if (!name) return;
      saveCurrentDataset(name);
      dlg.remove();
    });
  ...
  function saveCurrentDataset(name) {
    var cfg = state.config;
    var entry = {
      id: genMyId(),
      name: name,
      game: cfg.picks + "/" + cfg.poolSize + (cfg.bonusPool ? " + bonus" : ""),
      preset: presetKeyFromConfig(cfg) || "custom",
      poolMin: cfg.poolMin,
      poolMax: cfg.poolMax,
      picks: cfg.picks,
      bonusPool: cfg.bonusPool || 0,
      bonusPicks: cfg.bonusPicks || 0,
      draws: state.parsed.draws.length,
      dateRange: computeDateRange(state.parsed.draws),
      datedDraws: state.parsed.datedDrawsCount,
      schedule: "—",
      source: "User-saved (" + new Date().toISOString().slice(0, 10) + ")",
      sourceUrl: "",
      accessed: new Date().toISOString().slice(0, 10),
      note: "Saved from the draw history on " + new Date().toISOString().slice(0, 10) + ".",
      text: el.input.value,
      savedAt: Date.now()
    };
    var lib = getMyLibrary();
    lib.push(entry);
    setMyLibrary(lib);
    renderMyLibrary();
    // Brief confirmation
    var saveBtn = $("btnSaveDataset");
    if (saveBtn) { var t = saveBtn.textContent; saveBtn.textContent = "Saved ✓"; setTimeout(function () { saveBtn.textContent = t; }, 1800); }
  }

  function computeDateRange(draws) {
    var first = null, last = null;
    draws.forEach(function (d) {
      if (d.date) { if (!first || d.date < first) first = d.date; if (!last || d.date > last) last = d.date; }
    });
    return (first && last) ? (first + " to " + last) : "no dates";
  }

  function renderMyLibrary() {
    var host = $("myLibraryHost");
    var grid = $("myLibraryGrid");
    if (!host || !grid) return;
    var lib = getMyLibrary();
    if (!lib.length) { host.hidden = true; grid.innerHTML = ""; return; }
    host.hidden = false;
    lib.sort(function (a, b) { return (b.savedAt || 0) - (a.savedAt || 0); });
    grid.innerHTML = lib.map(function (d) {
      return (
        '<div class="library-card is-user" role="listitem" data-mysid="' + esc(d.id) + '">' +
          '<div class="lc-name">' + esc(d.name) + "</div>" +
          '<div class="lc-meta">' +
            "<span>" + esc(d.game) + "</span>" +
            "<span>" + esc(d.draws + " draws") + "</span>" +
          ...
          "</div>" +
          '<div class="lc-note">' + esc(d.note || "") + "</div>" +
          '<div class="lc-source">Saved ' + esc(d.accessed) + " &middot; this browser only</div>" +
          '<div class="lc-actions">' +
            '<button class="primary-button lc-load" type="button" data-myload="' + esc(d.id) + '">Load</button>' +
            '<button class="lc-delete" type="button" data-mydelete="' + esc(d.id) + '">Delete</button>' +
          "</div>" +
        "</div>"
      );
    }).join("");
    Array.prototype.forEach.call(grid.querySelectorAll("[data-myload]"), function (btn) {
      btn.addEventListener("click", function () { loadMyDataset(btn.getAttribute("data-myload"), btn); });
    });
    Array.prototype.forEach.call(grid.querySelectorAll("[data-mydelete]"), function (btn) {
      btn.addEventListener("click", function () { deleteMyDataset(btn.getAttribute("data-mydelete")); });
    });
  }

  function loadMyDataset(id, btn) {
    var lib = getMyLibrary();
    var ds = lib.filter(function (x) { return x.id === id; })[0];
    if (!ds) return;
    el.input.value = ds.text;
    applyDatasetConfig(ds);
    onInput();
    Array.prototype.forEach.call(document.querySelectorAll(".library-card"), function (c) { c.classList.remove("is-active"); });
    var card = document.querySelector('.library-card[data-mysid="' + cssEscape(id) + '"]');
    if (card) card.classList.add("is-active");
  ...
  function deleteMyDataset(id) {
    var lib = getMyLibrary();
    var ds = lib.filter(function (x) { return x.id === id; })[0];
    if (!ds) return;
    if (!confirm('Delete "' + ds.name + '" from my library? This cannot be undone.')) return;
    setMyLibrary(lib.filter(function (x) { return x.id !== id; }));
    renderMyLibrary();
  }


  var PRESETS = {
    "6/49": { poolMin: 1, poolMax: 49, picks: 6 },
    "7/52": { poolMin: 1, poolMax: 52, picks: 7 },
    "5/49": { poolMin: 1, poolMax: 49, picks: 5 },
    "6/45": { poolMin: 1, poolMax: 45, picks: 6 },
    "6/70": { poolMin: 1, poolMax: 70, picks: 6 },
    "4/0-9": { poolMin: 0, poolMax: 9, picks: 4 },
    "3/0-9": { poolMin: 0, poolMax: 9, picks: 3 },
    "2/0-9": { poolMin: 0, poolMax: 9, picks: 2 },
  };

  function presetKeyFromConfig(cfg) {
    for (var k in PRESETS) if (PRESETS.hasOwnProperty(k)) {
      var p = PRESETS[k];
      if (p.poolMin === cfg.poolMin && p.poolMax === cfg.poolMax && p.picks === cfg.picks) return k;
    }
    return null;
  }

  function presetFromValue(val) {
    return val in PRESETS ? PRESETS[val] : null;
  }
  ...
  function readConfig() {
    ...
    // Apply preset if selected; else keep manual fields intact.
    var presetVal = $("cfgPreset") ? $("cfgPreset").value : "custom";
    if (presetVal !== "custom" && presetFromValue(presetVal)) {
      var p = presetFromValue(presetVal);
      // Only overwrite if preset differs from current manual values (user-selected).
      // We still read from fields; preset change is handled by preset listener, not here.
    }
    $("priorNote").innerHTML =
  ...
  function writeConfig(cfg) {
    ...
    // Sync preset dropdown to match the active configuration.
    var presetEl = $("cfgPreset");
    if (presetEl) {
      var match = presetKeyFromConfig(cfg);
      presetEl.value = match ? match : "custom";
    }
    readConfig();
  }
  ...
  function onInput() {
    ...
    // Date awareness note (descriptive only): show when dates detected.
    el.parseStatus.className = cls;
    var dateNote = "";
    if (parsed.datedDrawsCount > 0) {
      dateNote = ' <span class="hint"><strong>' + parsed.datedDrawsCount + ' draws have dates</strong>. ' +
        'Date-derived cadence and interval diagnostics are descriptive statistics only — ' +
        'they do not establish predictive recurrence laws. ' +
        (parsed.dateIssues && parsed.dateIssues.length ? 'Note: ' + parsed.dateIssues.slice(0, 1).join("; ") + '.' :
         (parsed.dateIssues && parsed.dateIssues.length > 1 ? 'Notes: ' + parsed.dateIssues.slice(0, 2).join("; ") + '.' : '')) +
        '</span>';
    }
    el.parseStatus.innerHTML = msg + dateNote;
    $("btnAnalyze").disabled = n < 4;
    var saveBtn = $("btnSaveDataset");
    if (saveBtn) saveBtn.disabled = n < 1; // need at least one draw to save
    persist();
  }
  ...
  // Render date-aware descriptive diagnostics (only when dates present) in verdict.
  function dateAwareVerdictExtras(d) {
    if (!d || !d.dateStats || !d.dateStats.available) return "";
    var s = d.dateStats;
    var note = '<div class="card" style="margin-top:14px">' +
      '<div class="card-head"><div><h3>Date-aware descriptive statistics</h3>' +
      '<p><strong>Not predictive.</strong> These describe the observation timeline; they ' +
      'do not establish a predictive calendar-time recurrence law. No claim about future ' +
      'draw dates is made.</p></div></div>' +
      '<div class="grid-3">';
    note += metric("Dated draws", String(s.n), s.sorted ? "chronologically ordered" : (s.duplicates ? "contains duplicates" : s.outOfOrder ? "not in order" : "order unclear"), s.sorted ? "is-good" : "is-warn");
    note += metric("Duplicate dates", s.duplicates ? "yes — same date appears for multiple draws" : "none detected", s.duplicates ? "check input; dates should be unique per draw" : "good", s.duplicates ? "is-warn" : "is-good");
    note += metric("Chronological order", s.sorted ? (s.order === "descending" ? "newest-first (descending)" : "oldest-first (ascending)") : "draws not ordered (or duplicates present)", s.sorted ? "good for interval diagnostics" : "intervals not meaningful; sort before interpreting cadence", s.sorted ? "is-good" : (s.outOfOrder ? "is-warn" : "is-good"));
    note += '</div>';
    if (s.cadenceInfo && s.cadenceInfo.note) {
      note += '<p class="hint" style="margin-top:10px"><strong>Cadence note:</strong> ' +
        esc(s.cadenceInfo.note) + '</p>';
    }
    note += '</div>';
    return note;
  }
